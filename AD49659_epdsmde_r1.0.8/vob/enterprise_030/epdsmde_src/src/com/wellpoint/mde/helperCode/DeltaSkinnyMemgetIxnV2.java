...
import com.wellpoint.mde.Skinny.ProcessHmoSkinny;
import com.wellpoint.mde.Skinny.ProcessNetworkSkinny;
import com.wellpoint.mde.Skinny.ProcessOrgSkinny;
import com.wellpoint.mde.Skinny.ProcessProviderSkinny;
...
public class DeltaSkinnyMemgetIxnV2 {
	...
	private static final String PNET = "PNET";
 ...
 public void createConnection() {
    ...
    public void executeCall(MemHead memHead, Class<? extends AbstractMemgetIxn> clazz, boolean pnet ){
        ...
        //initDbConnection();
           ...
           ((AbstractMemgetIxn) obj).setMemGetProp(memGet);
           // Set entity management priority.
           // Default is 0.

           /* Issue MemGet */
           memgetStatus = memGet.execute(inpMemList, outMemList, GetType.ASENTITY, KeyType.MEMIDNUM, outAudRows);
           ...
           if(memgetStatus)
           {
        	   	  long entRecNum = outMemList.rowAt(0).getEntRecno();
                  ...
                  if(pnet){
                	  ((AbstractMemgetIxn) obj).setPnet(true);
                  }
                  outDataList = ((AbstractMemgetIxn) obj).processMemRows(outMemList, outAudRows, entRecNum);
           ...
           l_memHeadStatus = false;
                  ...
                  logInfo("MDE Error: legacy id = " + memHead.getMemIdnum());
                  logInfo(l_errorCd + ":" + l_errorTxt);
           }
    }

    public void execute_memGet(String sourceProvId, String sourceCode, String fileType) throws Exception
    {
           try{
    
           boolean pnet = false;
           MemHead memHead = new MemHead();
           memHead.setMemIdnum(sourceProvId);
           memHead.setSrcCode(sourceCode);
           if(null != fileType){
        	   pnet = PNET.equals(fileType.trim().toUpperCase());
           }
           if(sourceCode.contains("ORG")){
                     executeCall(memHead, ProcessOrgSkinny.class, pnet);     
               }
               else if(sourceCode.contains("NET") && !pnet){
                     executeCall(memHead, ProcessNetworkSkinny.class, false);
               }
               else if(sourceCode.contains("HMO") && !pnet){
                     executeCall(memHead, ProcessHmoSkinny.class, false);
               }
               else{
                     executeCall(memHead, ProcessProviderSkinny.class, pnet);
               }
           
           System.out.println("SourceProvider Id: " + sourceProvId +" Source Code: " + sourceCode + "; Size: " + l_memgetSize +
                   "; Memget time: " + l_memgetTime + " ms; Post Memget Time: " + l_postMemgetTime + " ms");

           //executeCall(memHead, true,fileType, null);
           }catch(Exception e){
                  l_errorCd = -1;
                  l_errorTxt = e.toString();
                  logInfo("MDE Error: legacy id = " + sourceProvId);
                  logInfo(l_errorCd + ":" + l_errorTxt);
           }
    ...
    public static void main(String[] args) {
    	DeltaSkinnyMemgetIxnV2 jtx = new DeltaSkinnyMemgetIxnV2();
  		try	{
  			
  			jtx.execute_memGet("10000000079", "EPDSV2ORG", "");
  		}
  		catch (Exception ex)	{
  			ex.printStackTrace();
  		}
